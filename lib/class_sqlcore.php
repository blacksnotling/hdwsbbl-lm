...
public static function installProcsAndFuncs($install = true)
{
    ...
    foreach ($routines as $r) {
        $status &= (mysql_query($r) or die("<br><strong><font color='red'>FATAL ERROR</font></strong>: One or more OBBLM MySQL functions/procedures could not be created. This error is <strong>most likely</strong> due to your database user NOT having the \"CREATE ROUTINE\" privilege. Some web hosts are willing to help you work around this problem by running this install/upgrade script for you. If not, you will have to find another web host allowing \"CREATE ROUTINE\".<br><br>\n<strong>MySQL error (errno ".mysql_errno()."):</strong><br>". mysql_error().'<br><br><strong>The function/procedure SQL code that failed was:</strong><br>'.$r.'<br><br><strong>Need help?</strong> Try seeking help at the <a TARGET="_blank" href="http://code.google.com/p/obblm/issues/list">OBBLM developers site</a>'));
    }
