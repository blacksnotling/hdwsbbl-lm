...
*/
10  => array('<table border="1"><tr><td style="color:darkred; font-size:1.2em;"><strong>JpGraph Error:</strong>
HTTP headers have already been sent.<br>Caused by output from file <strong>%s</strong> at line <strong>%d</strong>.</td></tr><tr><td><strong>Explanation:</strong><br>HTTP headers have already been sent back to the browser indicating the data as text before the library got a chance to send it\'s image HTTP header to this browser. This makes it impossible for the library to send back image data to the browser (since that would be interpretated as text by the browser and show up as junk text).<p>Most likely you have some text in your script before the call to <em>Graph::Stroke()</i>. If this texts gets sent back to the browser the browser will assume that all data is plain text. Look for any text, even spaces and newlines, that might have been sent back to the browser. <p>For example it is a common mistake to leave a blank line before the opening "<strong>&lt;?php</strong>".</td></tr></table>',2),
